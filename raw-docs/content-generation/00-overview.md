...
2. **Set `CRON_SECRET` to a strong random value** in both `.env.local` and Vercel before deploying. Without it, the inactivity cron endpoint is effectively unprotected.

3. **Tighten RLS policies.** Current policies grant access to all `authenticated` users. Before adding any client-facing Supabase auth, update the policies to check `admins` table membership:
   ```sql
   USING (EXISTS (SELECT 1 FROM admins WHERE admins.id = auth.uid()))
   ```

4. **Add Basecamp OAuth CSRF protection.** The OAuth flow is missing a `state` parameter. Generate a random state in `/api/basecamp/auth`, store it in a cookie, and validate it in `/api/basecamp/callback`.
...
| Research job stuck >10 minutes | `/api/content-jobs/[id]/status` endpoint detects and marks stuck rows as `error`. UI shows "Resume Research" button. |
