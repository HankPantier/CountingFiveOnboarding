...
function phase3Instructions(session: Session, mode: AgentMode): string {
  ...
  const subReviewPending = isSubCategoryReviewPending(typedMeta, niches)
...
Accept everything in any format. As soon as it lands, call update_session_data with all populated fields, resolvedGaps including "culture.linkedIn.url" and "business.googleBusinessProfile.url", and "_meta": { "phase3_completed_chunks": ["chunk1"] }.${chunk1Analyst}`
    }
...
When part 1 is done, call update_session_data with the structured fields populated and resolvedGaps including "culture.linkedIn.url" and "business.googleBusinessProfile.url"; updates must include { "_meta": { "phase3_completed_chunks": ["chunk1"] } }.${chunk1Analyst}`
  }
...
Tier 1 gaps MUST be answered before advancing (server-side enforced). Tier 1 now includes client success stories, a voice sample, and each kept niche's buying trigger + value prop — if the firm genuinely has nothing for one, write the string "None" to that field and list it in resolvedGaps (a gap only resolves when its field holds a value). If the staff member skips a Tier 1 item they DO have, ask for it again in a short follow-up. Tier 2 / Tier 3 may be skipped — for Tier 3, log into "_meta.phase4_flagged_for_followup".
