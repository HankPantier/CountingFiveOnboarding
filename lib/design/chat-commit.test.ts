...
describe('commitWorkspace', () => {
  ...
  it('refuses an unpreviewed edit made AFTER a failed preview (commit_version and the auto-commit)', async () => {
    const ws = workspace()
    ws.apply({ kind: 'palette', patch: { primary: '#123a5c' } })
    ws.recordPreview({ metrics: OVERFLOW, baseline: CLEAN, shots: [] })
    ws.apply({ kind: 'css', target: 'global', css: ':root { --c5-probe: 1px; }' })
    expect(ws.currentPreview()).toBeNull()
    const out = await commitWorkspace(ws, { summary: 'x', target: TARGET, commitVersion })
    expect(out.ok).toBe(false)
    expect(!out.ok && out.failures?.[0]).toContain('wider than the screen')
    const auto = await finishTurnCommit(ws, (summary) => commitWorkspace(ws, { summary, target: TARGET, commitVersion }), false)
    expect(auto).toMatchObject({ status: 'blocked' })
    expect(commitVersion).not.toHaveBeenCalled()
    // A clean preview of the NEW revision lifts the block.
    ws.recordPreview({ metrics: CLEAN, baseline: CLEAN, shots: [] })
    expect(await commitWorkspace(ws, { summary: 'x', target: TARGET, commitVersion })).toMatchObject({ ok: true, warnings: [] })
  })
  it('a slow preview of an older revision never overwrites a newer preview', () => {
    const ws = workspace()
    ws.apply({ kind: 'palette', patch: { primary: '#123a5c' } })
    ws.recordPreview({ metrics: CLEAN, baseline: CLEAN, shots: [] })
    ws.recordPreview({ metrics: OVERFLOW, baseline: CLEAN, shots: [] }, 0)
    expect(ws.latestPreview()?.metrics).toBe(CLEAN)
  })
