...
describe('Vercel packaging (R7)', () => {
  ...
  const HEAVY = /^import[^\n]*from '@\/lib\/design\/(css-sanitizer|bundle-files|apply-bundle|commit-version|chat-workspace|chat-preview|chat-tools|chat-commit|concept-validate|concept-generator|run-orchestrator|model-call|critic|concept-reviser|run-gather|refine-stage|render\/render-composed|render\/render-folds)'/m
  it.each([
