// ---------------------------------------------------------------------------
// Pricing-calculator ⇄ client repo sync.
//
// For already-published clients (github_repo set, phase ≥ 6) the calculator's
// live config lives as content/pricing-calculator.json in the repo — the site
// reads it at build time. So the admin config editor must (a) SEED from that
// file when there's no DB row yet, and (b) PUSH edits back to the draft branch
// on save, so "Publish to live" in the content editor deploys them. Onboarding
// (pre-publish) sessions keep using the DB row + package-time emission.
// ---------------------------------------------------------------------------
...
import {
  ...
  type PricingCalculatorRecord,
} from './pricing-calculator-config'
import {
  buildPricingCalculatorPageMd,
  PRICING_CALCULATOR_JSON_PATH,
  PRICING_CALCULATOR_PAGE_PATH,
...
// Read the live calculator config from the repo (draft branch preferred, then main).
export async function readPricingCalculatorFromRepo(
  githubRepo: string
): Promise<PricingCalculatorConfig | null> {
  for (const branch of [DRAFT_BRANCH, MAIN_BRANCH]) {
    try {
      const file = await readFile(githubRepo, PRICING_CALCULATOR_JSON_PATH, branch)
      return normalizePricingConfig(JSON.parse(file.content))
    } catch (err) {
...
export type LoadedPricingCalculator = PricingCalculatorRecord & {
...
export async function loadPricingCalculatorForSession(
  sessionId: string
): Promise<LoadedPricingCalculator> {
  ...
  const db = await getPricingCalculator(sessionId)
...
async function setNavEntry(repo: string, present: boolean, author: Author): Promise<void> {
  ...
  const has = parsed.primary.some(i => i.url === PRICING_CALCULATOR_URL)
  if (present && !has) parsed.primary.push({ label: PRICING_CALCULATOR_NAV_LABEL, url: PRICING_CALCULATOR_URL })
  else if (!present && has) parsed.primary = parsed.primary.filter(i => i.url !== PRICING_CALCULATOR_URL)
  else return
  await writeFile(repo, NAV_PATH, serializeNavJson(parsed), DRAFT_BRANCH, present ? 'Add pricing calculator to nav' : 'Remove pricing calculator from nav', { expectedSha: nav.sha, ...author })
}

// Push the calculator to the repo draft branch so it reaches the site. When
...
export async function syncPricingCalculatorToRepo(args: {
  githubRepo: string
  ...
  try {
    ...
    if (enabled) {
      await upsertFile(githubRepo, PRICING_CALCULATOR_JSON_PATH, JSON.stringify(config, null, 2), 'Update pricing calculator config', author)
      try {
        await readFile(githubRepo, PRICING_CALCULATOR_PAGE_PATH, DRAFT_BRANCH)
      } catch (err) {
        if (err instanceof FileNotFoundError) {
          await writeFile(githubRepo, PRICING_CALCULATOR_PAGE_PATH, buildPricingCalculatorPageMd(firmName, config), DRAFT_BRANCH, 'Add pricing calculator page', author)
        } else throw err
      }
      await setNavEntry(githubRepo, true, author)
    } else {
      await removeIfExists(githubRepo, PRICING_CALCULATOR_JSON_PATH, 'Remove pricing calculator config', author)
      await removeIfExists(githubRepo, PRICING_CALCULATOR_PAGE_PATH, 'Remove pricing calculator page', author)
