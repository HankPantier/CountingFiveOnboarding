...
export default function AuditChat({
  ...
  const transport = useMemo(
    () => new DefaultChatTransport({ api: `/api/audits/${auditId}/chat` }),
    [auditId]
  )
  ...
  // After each completed exchange, refresh the server component so the report
  // reflects any edit_audit tool writes.
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full border border-border-default rounded-lg bg-surface-card overflow-hidden">
      <div className="px-4 py-3 border-b border-border-default bg-surface-subtle">
        <h2 className="text-sm font-heading font-semibold text-text-primary">Report Edit Assistant</h2>
        <p className="text-xs text-text-muted font-body mt-0.5">
          Direct edits to the AI intelligence sections — scores stay read-only.
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[300px]">
        {messages.length === 0 && (
          <p className="text-text-muted font-body text-sm italic">
            Try: &ldquo;Expand the church line item under Niche &amp; Services&rdquo; or &ldquo;Sharpen the competitive commentary.&rdquo;
          </p>
        )}
        {messages.map(m => {
          const text = m.parts
