...
export default async function PricingPlansPage({
  ...
  const record = await loadPricingPlansForSession(id)
  ...
  return (
    <main className="p-8 max-w-[1000px] mx-auto">
      <div className="mb-8">
        ...
        <div className="mt-4">
          <h1 className="text-2xl font-heading font-bold text-brand-navy">Plans &amp; pricing</h1>
          <p className="text-text-secondary font-body text-sm mt-1">
            {firmName ?? session.website_url} · configure the static plans/pricing page shown on the client site
          </p>
        </div>
      </div>
