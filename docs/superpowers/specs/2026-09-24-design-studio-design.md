...
- **Accepted deviations (recorded 2026-09-26, T2 + P6b):**
  ...
  - **`DesignBundle.style` is canonical** (defaults dropped; absent = all default). Apply writes the whole axis set, deleting `design.json.style` when all default. Below L3 (effective tier) the site's current style is held: the generator restores the draft's current `style` (with a note only when the concept tried to change it), and apply rejects only a bundle that would change it. An absent `style` below L3 (pre-P6b versions/concepts) means "keep current" — `keepLockedStyle` fills it from the draft before render, so a restore never wipes the site's axes. At L3+ absent still means all default.
